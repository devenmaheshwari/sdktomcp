...
class SDKIntrospector:
    ...
    def __init__(self, sdk_module_name: str):
    ...
    def discover_methods(self, max_depth: int = 3) -> List[SDKMethod]:
        ...
        try:
            ...
            logger.info(f"Successfully imported {self.sdk_module_name}")
            
            # Recursively explore the module
            self._explore_module(sdk_module, self.sdk_module_name, depth=0, max_depth=max_depth)
            
            logger.info(f"Discovered {len(self.discovered_methods)} methods")
            return self.discovered_methods
            
        except ImportError as e:
            logger.error(f"Failed to import {self.sdk_module_name}: {e}")
            return []
    ...
    def _explore_module(self, module, module_path: str, depth: int, max_depth: int):
        ...
        for name, obj in inspect.getmembers(module):
            ...
            # Explore classes
            if inspect.isclass(obj) and obj.__module__.startswith(self.sdk_module_name):
                self._explore_class(obj, full_path, depth + 1, max_depth)
            
            # Explore submodules
            elif inspect.ismodule(obj) and obj.__name__.startswith(self.sdk_module_name):
                if depth < max_depth:
    ...
    def _is_useful_method(self, method: SDKMethod) -> bool:
        ...
        action_keywords = [
            'create', 'delete', 'update', 'get', 'list', 'fetch', 'retrieve',
            'add', 'remove', 'modify', 'set', 'configure', 'deploy', 'scale',
            'start', 'stop', 'restart', 'pause', 'resume', 'execute', 'run'
        ]
        
        method_lower = method.name.lower()
        if any(keyword in method_lower for keyword in action_keywords):
            return True
        
        # Include methods with good docstrings
...
class LLMAnalyzer:
    ...
    def analyze_methods(self, methods: List[SDKMethod], sdk_name: str) -> List[MCPTool]:
        ...
        # Process methods in batches to avoid token limits
        batch_size = 5
    ...
    def _create_analysis_prompt(self, method_info: List[Dict], sdk_name: str) -> str:
        ...
        methods_json = json.dumps(method_info, indent=2)
        
        return f"""
Analyze these {sdk_name} SDK methods and convert them to MCP tool definitions.

For each method, provide:
1. A clear, descriptive tool name (use snake_case)
2. A concise description of what the tool does
3. JSON schema for parameters (required and optional)
4. A category (e.g., "resource_management", "monitoring", "configuration")

Methods to analyze:
{methods_json}

Respond with a JSON array of tool definitions in this format:
[
  {{
    "name": "tool_name",
...
Focus on making the tools intuitive for AI agents to use. Consolidate similar methods if appropriate.
"""
...
class SDKToMCPConverter:
    ...
    def convert(self, sdk_name: str, output_dir: str = "./generated_mcp_servers") -> str:
        ...
        if not methods:
            raise ValueError(f"No methods found in SDK: {sdk_name}")
