...
class LLMAnalyzer:
    ...
    def analyze_methods(self, methods: List[SDKMethod], sdk_name: str) -> List[MCPTool]:
        ...
        # Much more aggressive filtering - only keep the most essential methods
        max_methods = 20  # Reduced from 100 to 20 for manageable output
        ...
        # Process methods in smaller batches
        batch_size = 3
    ...
    def _select_essential_methods(self, methods: List[SDKMethod], limit: int) -> List[SDKMethod]:
        ...
        # Define essential operations for each SDK
        essential_patterns = {
            'kubernetes': [
                ...
                'list_secret', 'create_namespaced_secret'
            ],
            'github': [
...
This is a template that requires customization for your specific use case.
You'll need to:
1. Initialize the {sdk_name} client properly
...
# SDK imports
try:
    {self._generate_sdk_imports(sdk_name)}
except ImportError as e:
    print(f"Please install the {sdk_name} SDK")
    exit(1)
...
# TODO: Initialize your {sdk_name} client here
# Example:
# client = initialize_{sdk_name.lower()}_client()
"""

        # Generate simplified tool definitions
        tool_definitions = []
        for tool in tools:
            tool_def = f'''Tool(
    name="{tool.name}",
    description="{tool.description[:100]}...",  # Truncated for readability
    input_schema={json.dumps(self._simplify_schema(tool.parameters), indent=2)}
)'''
            tool_definitions.append(tool_def)
        
        tools_list = ',\n    '.join(tool_definitions)
        
        # Generate consolidated handler instead of individual handlers
        handler_code = self._generate_consolidated_handler(tools, sdk_name)
...
"""
    ...
    def _generate_sdk_imports(self, sdk_name: str) -> str:
        """Generate appropriate imports for each SDK"""
        imports = {
            'kubernetes': """
    ...
    # github_client = Github("your-token-here")""",
            'azure': """
    ...
    # compute_client = ComputeManagementClient(credential, "subscription-id")"""
        ...
        return imports.get(sdk_name.lower(), f"import {sdk_name.lower()}")
    
    def _simplify_schema(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        """Simplify complex schemas to avoid massive JSON objects"""
        ...
        important_props = list(properties.items())[:5]
        
        for prop_name, prop_def in important_props:
            simplified["properties"][prop_name] = {
                "type": prop_def.get("type", "string"),
                "description": prop_def.get("description", f"Parameter {prop_name}")[:50]
            }
        
        return simplified
    
    def _generate_consolidated_handler(self, tools: List[MCPTool], sdk_name: str) -> str:
        """Generate a single consolidated handler instead of many individual ones"""
        tool_cases = []
        for tool in tools:
            case = f'''    elif name == "{tool.name}":
        # TODO: Implement {tool.name}
        # Method: {tool.method_path}
        result = {{"message": "Tool {tool.name} not yet implemented", "arguments": arguments}}'''
            tool_cases.append(case)
...
async def handle_tool_call(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    \"\"\"Handle all tool calls - consolidated handler\"\"\"
    try:
